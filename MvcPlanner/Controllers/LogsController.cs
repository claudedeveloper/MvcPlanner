...
namespace MvcPlanner.Controllers
{
    public class LogsController : Controller
    {
        ...
        {
            var logs = from m in db.Logs
                       select m;
            
            if (!String.IsNullOrEmpty(searchString))
            {
                logs = logs.Where(s => s.Subject.Contains(searchString));
            }
            return View(logs);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,Subject,Remark,Day,Date,Assignment,Time,Recitation,Comment,Quiz,Seatwork,Midterm,Final")] Log log)
        {
            if (ModelState.IsValid)
            {
